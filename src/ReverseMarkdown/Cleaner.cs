...
#else
    // .NET Framework 4.8 fallback - use regular Regex
    private static readonly Regex _slackBoldCleaner = new Regex(@"\*(\s\*)+", RegexOptions.Compiled);
    private static Regex SlackBoldCleaner() => _slackBoldCleaner;

    private static readonly Regex _slackItalicCleaner = new Regex(@"_(\s_)+", RegexOptions.Compiled);
    private static Regex SlackItalicCleaner() => _slackItalicCleaner;

    private static readonly Regex _nonBreakingSpaces = new Regex(@"[\u0020\u00A0]", RegexOptions.Compiled);
